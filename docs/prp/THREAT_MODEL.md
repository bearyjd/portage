...
| 14 | Malicious sender abuses **default-app role restore** (#122): aiming a privileged `cmd role add-role-holder` at a role or package of its choosing, or making portage claim a default it never set | **The ROLE is unrepresentable as attacker input**: it rides as a closed `@SerialName` enum (`RestorableRole`), non-nullable with no default and no `coerceInputValues`, so an unknown role fails deserialization and the WHOLE item is rejected — a hostile sender cannot name `ASSISTANT`/`CALL_REDIRECTION`/`SMS`. `:adb-bridge` holds an independent `RoleTarget` enum joined by an exhaustive `when`, and `:providers` has no `:adb-bridge` dependency, so the closure is structural rather than validated; both enums carry closed-set tripwire tests. **Flag injection** is stopped by the caller's package grammar (full match on dot-separated `[A-Za-z0-9_]`), NOT by `ShellArgs` — whose SAFE_ARG allowlist includes `-` and would pass `--user` through unquoted. **Consent**: the apply provider holds no `RoleRestorer` reference at all (restoring is structurally impossible there); `DEFAULT_ROLES` is Tier 1 so it is not pre-checked on the checklist; each role needs its own explicit tap, and there is no "restore all". **No false success**: exit 0 is not trusted — the role is read back (`cmd role get-role-holders`) and an unverifiable readback reports UNAVAILABLE, never RESTORED | The **package is fully attacker-chosen** — accepted residual (§3). Unlike the relay (row 13) the target cannot be derived from a typed enum, because browsers/dialers/launchers are not enumerable. The real constraints are exactly three: syntactically a package name, installed on this device, and explicitly tapped by the user with the package name shown. A sender can therefore offer the user a role pointing at any installed app; it cannot apply it, and the platform independently refuses a non-qualifying package |
