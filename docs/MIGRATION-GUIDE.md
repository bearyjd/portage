...
|---|---|---|
| Contacts | **Portage** | vCard transfer into device-local contacts, including the favorite/starred bit; exact retries and existing raw contacts are deduplicated. Account-backed contacts may instead reappear through their sync provider. |
