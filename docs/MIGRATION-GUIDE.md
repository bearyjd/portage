...
|---|---|---|
| Contacts | **Portage** | vCard transfer into device-local contacts, including the favorite/starred bit and bounded contact thumbnails; exact retries and existing raw contacts are deduplicated. Full-resolution photos and app-specific raw-contact data are not portable. Account-backed contacts may instead reappear through their sync provider. |
